...
reco_uv <- Recommender(rmat, "SVDF", 
      param=list(k = 4, gamma = 0.015, lambda = 0.001, min_improvement = 1e-06, min_epochs = 50, max_epochs = 200))
...
fsvd <- funkSVD(x = rmat, k = 4, gamma = 0.015, lambda = 0.001, min_improvement = 1e-06, min_epochs = 50, max_epochs = 200, verbose = FALSE)
...
# convert the jester data frame to a matrix
netflix_small_rmat <- as.matrix(netflix_small)

# convert matrix to a recommenderlab realRatingMatrix
netflix_small_rmat <- as(netflix_small_rmat,"realRatingMatrix")
```

```{r}
# convert the jester data frame to a matrix
netflix_rmat <- as.matrix(netflix)
...
```{r}
#netflix_small_rmat@data
#colSums(is.na(netflix_small))

netflix_small[rowSums(is.na(netflix_small))==length(netflix_small),]
```
```{r}
netflix_small_2 <- netflix_small[rowSums(is.na(netflix_small))<40,]

# convert the jester data frame to a matrix
netflix_small_rmat_2 <- as.matrix(netflix_small_2)

# convert matrix to a recommenderlab realRatingMatrix
netflix_small_rmat_2 <- as(netflix_small_rmat_2,"realRatingMatrix")
```


```{r}
# split the data into the training and the test set:
split_data <- evaluationScheme(netflix_small_rmat_2, method="split", train=0.8, k=1, given=10, goodRating=3)
```
...
```{r}
class(getData(split_data, "known"))
```

```{r}
netflix_predictions <- predict(netflix_reco, getData(split_data, "known"), type="ratings")
netflix_predictions@data
```

```{r}
calcPredictionAccuracy(netflix_predictions, getData(split_data, "unknown"))
```
